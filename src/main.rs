...
fn solve() -> Option<(String, usize)> {
    ...
    let mut previous_guesses: Vec<(String, Vec<String>)> = vec![];
    loop {
        let guess = algorithms::brute_force_indexed(&word_list, &previous_guesses, &master_word_list);
        rounds += 1;
        ...
        algorithms::reduce_word_list(&mut word_list, &guess, results.clone());
        if let Some(index) = &master_word_list.iter().position(|word| word == &guess) {
            master_word_list.remove(*index);
        }
        previous_guesses.push((guess, results));
